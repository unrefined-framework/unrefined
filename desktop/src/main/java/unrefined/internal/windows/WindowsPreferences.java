...
public class WindowsPreferences extends Preferences {
    ...
    public static boolean delete(String namespace, String name) {
        ...
        synchronized (preferencesMap) {
            ...
            if (preferences != null) RegCloseKey(preferences.hKey);
            long lpSubKey = ForeignSupport.allocateWideCharString(key);
            try {
                int code = RegDeleteKeyW(HKEY_CURRENT_USER, lpSubKey);
                return code == 0;
            }
            finally {
    ...
    private static long openHKey(String name) {
        ...
        try {
            int errno;
            if ((errno = RegCreateKeyExW(HKEY_CURRENT_USER, lpSubKey, 0, 0,
        ...
        private void flush() {
            synchronized (this) {
                for (String key : removed) {
                    ...
                    try {
                        if (RegQueryValueExW(preferences.hKey, lpValueName, 0, 0, 0, 0) == 0) {
                            int errno;
                            if ((errno = RegDeleteValueW(preferences.hKey, lpValueName)) != 0) {
                ...
                for (Map.Entry<String, Map.Entry<String, String>> entry : cache.entrySet()) {
                    ...
                    try {
                        ...
                        if (RegQueryValueExW(preferences.hKey, lpValueName, 0, lpType, 0, lpcbData) == 0) {
                            if (MEMORY_IO.getInt(lpType) == 0x00000001 /* REG_SZ */) {
                                ...
                                if (cbData < OSInfo.WIDE_CHAR_SIZE) changed = !value.isEmpty();
                                    ...
                                    try {
                                        ...
                                        if (RegQueryValueExW(preferences.hKey, lpValueName, 0, lpType, lpData, lpcbData) == 0) {
                        ...
                        if (changed) {
                            ...
                            try {
                                int errno;
                                if ((errno = RegSetValueExW(preferences.hKey, lpValueName, 0, 0x00000001 /* REG_SZ */, lpData, cbData)) != 0) {
                                    throw new UnexpectedError(new LastErrorException(errno));
                                }
                                preferences.onChange().emit(new ChangeEvent(preferences, key));
                            }
    ...
    private void getValueNames(Set<String> target) {
        ...
        try {
            int errno;
            if ((errno = RegQueryInfoKeyW(hKey, 0, 0, 0, 0, 0,
                ...
                try {
                    for (int dwIndex = 0; dwIndex < cValues; dwIndex ++) {
                        if ((errno = RegEnumValueW(hKey, dwIndex, lpValueName, lpcbMaxValueNameLen, 0, 0, 0, 0)) == 0) {
    ...
    private String query(String key, String defaultValue) {
        ...
        long lpcbData = UNSAFE.allocateMemory(Type.UINT32.size());
        try {
            if (RegQueryValueExW(hKey, lpValueName, 0, lpType, 0, lpcbData) == 0) {
                if (MEMORY_IO.getInt(lpType) == 0x00000001 /* REG_SZ */) {
                    int cbData = MEMORY_IO.getInt(lpcbData);
                    if (cbData < OSInfo.WIDE_CHAR_SIZE) return "";
                        ...
                        try {
                            UNSAFE.setMemory(lpData + cbData, OSInfo.WIDE_CHAR_SIZE, (byte) '\0');
                            if (RegQueryValueExW(hKey, lpValueName, 0, lpType, lpData, lpcbData) == 0) {
    ...
    public boolean contains(String key) {
        long lpValueName = ForeignSupport.allocateWideCharString(key);
        try {
            return RegQueryValueExW(hKey, lpValueName, 0, 0, 0, 0) == 0;
        }
        finally {
            UNSAFE.freeMemory(lpValueName);
        }
