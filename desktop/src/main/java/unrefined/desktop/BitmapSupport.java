...
public final class BitmapSupport {

    private BitmapSupport() {
        throw new NotInstantiableError(BitmapSupport.class);
    }
    ...
    public static BufferedImage deriveImage(BufferedImage bufferedImage, int x, int y, int width, int height) {
        ...
        if (raster == null) return bufferedImage.getSubimage(x, y, width, height);
                ...
                AWTSupport.getProperties(bufferedImage));
    }
    ...
    public static BufferedImage createImage(int width, int height, int bitmapType) {
        switch (bitmapType) {
            ...
            case ALPHA_8:
                ColorModel model1 = new DirectColorModel(COLOR_SPACE_SRGB, 8,
                        0, 0, 0, 0xFF,
                        false, DataBuffer.TYPE_BYTE);
                return new BufferedImage(model1, model1.createCompatibleWritableRaster(width, height), false, null);
    ...
    public static int getBitmapType(BufferedImage bufferedImage) {
        switch (bufferedImage.getType()) {
            case BufferedImage.TYPE_INT_ARGB: return RGBA_8888;
