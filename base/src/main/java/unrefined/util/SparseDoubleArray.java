...
public class SparseDoubleArray implements Cloneable, Iterable<Double>, RandomAccess {
    ...
    public SparseDoubleArray clone() {
        SparseDoubleArray clone;
        try {
            clone = (SparseDoubleArray) super.clone();
        } catch (CloneNotSupportedException e) {
            clone = new SparseDoubleArray();
        }
    ...
    public int indexOfValue(double value) {
        for (int i = 0; i < size; i ++) {
            if (Double.compare(values[i], value) == 0) return i;
        }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof SparseDoubleArray)) return false;

        SparseDoubleArray that = (SparseDoubleArray) o;

        if (size != that.size) return false;
        for (int i = 0; i < size; i ++) {
            if (Double.compare(keys[i], that.keys[i]) != 0) return false;
            if (Double.compare(values[i], that.values[i]) != 0) return false;
        }
        return true;
    }
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < size; i ++) {
            ...
            long temp = Double.doubleToLongBits(values[i]);
