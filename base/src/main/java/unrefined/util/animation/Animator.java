...
/**
 * The Animator interface lets you interpolate any attribute from any
 * object. Just implement it as you want and register it to the engine by
 * calling {@link Tween#registerAnimator(Class, Animator)}.
 * <p/>
 *
 * <h2>Example</h2>
 *
 ...
 *
 * <pre> {@code
 * public class ParticleAnimator implements Animator<Particle> {
 ...
 * }</pre>
 *
 * Once done, you only need to register this Animator once to be able to
 * use it for every Particle objects in your application:
 * <p/>
 *
 * <pre> {@code
 * Tween.registerAnimator(Particle.class, new ParticleAnimator());
 * }</pre>
 *
...
public abstract class Animator<T> {
	 ...
	 */
    public abstract int retrieve(T target, int tweenType, float[] returnValues);
	 ...
	 */
    public abstract void animate(T target, int tweenType, float[] newValues);
