...
public class ScheduledTimer {
    ...
    public final static class Builder {
        ...
        public ScheduledTimer unstarted() {
            return timer;
        }
    }
    ...
    private static final ScheduledExecutorService EXECUTOR = Executors.newSingleThreadScheduledExecutor(runnable -> {
        Thread thread = new Thread(runnable, "ScheduledTimer");
    ...
    public ScheduledTimer(long delay) {
        this(delay, 0);
    }

    public ScheduledTimer(long delay, long period) {
        this(delay, period, true);
    }

    public ScheduledTimer(long delay, long period, boolean repeat) {
        this(delay, period, repeat, true);
    }

    public ScheduledTimer(long delay, long period, boolean repeat, boolean coalesce) {
        this(delay, period, repeat, coalesce, null);
    }

    public ScheduledTimer(long delay, long period, boolean repeat, boolean coalesce, TimeUnit timeUnit) {
        setDelay(delay);
        setPeriod(period);
        this.repeat = repeat;
